...
def make_bot(cfg: dict) -> tuple:
    ...
    # ═══════════════════════════════════════════════════
    #  /update — git pull + pip install + restart (SUPER OWNER / MAIN OWNER only)
    # ═══════════════════════════════════════════════════
    @app.on_message(filters.command("update"))
    async def cmd_update(_, msg: Message):
        if not msg.from_user: return
        uid = msg.from_user.id
        # Only SUPER OWNER (env OWNER_ID) — never clones / normal owners
        if uid != PRIMARY["original_owner_id"]:
            await msg.reply_text(BAKA, parse_mode=enums.ParseMode.HTML)
            return

        sm = await msg.reply_text(
            "⏳ <b>Update Started…</b>\n\n"
        ...
        # ── Step 1: git pull ───────────────────────────
        rc, out = await run("git pull origin main 2>&1")
        git_icon = "✅" if rc == 0 else "❌"
        git_log  = out[:600] if out else "(no output)"
        logger.info(f"[UPDATE] git pull rc={rc}\n{git_log}")

        if rc != 0:
            await sm.edit_text(
                f"❌ <b>git pull failed (rc={rc})</b>\n\n<pre>{git_log}</pre>",
                parse_mode=enums.ParseMode.HTML,
            )
            return

        await sm.edit_text(
            f"{git_icon} <b>Git Pull:</b> <code>rc={rc}</code>\n<pre>{git_log[:400]}</pre>\n\n"
        ...
        # ── Step 2: pip install ────────────────────────
        rc2, out2 = await run(
            "pip install -r requirements.txt --quiet --no-cache-dir 2>&1"
        )
        pip_icon = "✅" if rc2 == 0 else "⚠️"
        pip_log  = out2[:600] if out2 else "(no output)"
        logger.info(f"[UPDATE] pip install rc={rc2}\n{pip_log}")

        await sm.edit_text(
            f"{git_icon} <b>Git Pull:</b> done\n"
            f"{pip_icon} <b>pip install:</b> <code>rc={rc2}</code>\n<pre>{pip_log[:300]}</pre>\n\n"
            "♻️ Restarting bot…",
            parse_mode=enums.ParseMode.HTML,
        )

        logger.info("[UPDATE] Restarting process…")

        # ── Step 3: restart (exec replaces process — MongoDB stays untouched) ─
        import sys
        await asyncio.sleep(1)          # let the edit land
