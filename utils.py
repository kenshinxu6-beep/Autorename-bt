...
def parse_info(caption, filename):
    """
    Extract metadata from caption and filename.
    Returns dict with keys: season, episode, quality, audio, name, video_length
    """
    info = {
        "season": "01",
        "episode": "01",
    ...
    # Season: "Season: 01" or "S01"
    s_match = re.search(r"(?:Season|S)\s*[:\-]?\s*(\d+)", cap, re.I)
    if s_match:
        info["season"] = s_match.group(1).zfill(2)
    
    # Episode: "Episode: 24" or "E24"
    e_match = re.search(r"(?:Episode|Ep|E)\s*[:\-]?\s*(\d+)", cap, re.I)
    if e_match:
        info["episode"] = e_match.group(1).zfill(2)
    
    # Quality: 480p, 720p, 1080p
    q_match = re.search(r"(\d{3,4}p)", cap)
    if q_match:
        info["quality"] = q_match.group(1)
    
    # Audio: "Audio: [Hindi-Tamil-Telugu-Eng-Jap]" or "Language - Hindi"
    ...
    # Name: "ᴀɴɪᴍᴇ: Jujutsu Kaisen" or first clean line
    name_match = re.search(r"(?:ᴀɴɪᴍᴇ|Anime|Name)\s*:\s*(.+)", cap, re.I)
    if name_match:
        info["name"] = name_match.group(1).strip()
    else:
        # Fallback: clean first non-empty line
...
def new_filename(info, template=None):
    """
    Generate new filename from template with placeholders.
    Default template: {name} S{season}E{episode} [{audio}] [{quality}]
    """
    if not template:
    ...
    # Remove leading/trailing spaces around brackets
