...
def make_bot(cfg: dict) -> tuple:
    ...
    async def send_result(msg: Message, anime: dict):
        ...
        if anime.get("mode") == "sticker" and anime.get("sticker_id"):
            ...
            kb = InlineKeyboardMarkup([[InlineKeyboardButton(btn_label, url=btn_url)]]) if (btn_label and btn_url) else None
            try:
                # Send sticker with inline button as reply_markup directly
                await app.send_sticker(
                    msg.chat.id,
                    sticker=anime["sticker_id"],
                    reply_markup=kb,
                )
            except Exception:
                # Fallback: send sticker alone then button separately
                try:
                    await app.send_sticker(msg.chat.id, sticker=anime["sticker_id"])
                except Exception:
                    pass
                if kb:
                    await msg.reply_text(btn_label, reply_markup=kb)
            return

        # ── NORMAL MODE ───────────────────────────────────
        desc = anime.get("description", "")
        img  = anime.get("image_file_id")
        # Build keyboard — buttons field is list of rows (each row = list of dicts)
        raw_btns = anime.get("buttons", [])
        kb_rows  = []
        if raw_btns:
            # Support both old flat format [{label,url}] and new nested [[{label,url}]]
            for row in raw_btns:
                if isinstance(row, list):
                    # new format: row is a list of button dicts
                    kb_rows.append([InlineKeyboardButton(b["label"], url=b["url"]) for b in row if b.get("url")])
                elif isinstance(row, dict) and row.get("url"):
                    # old flat format: each item is a single button → one per row
                    kb_rows.append([InlineKeyboardButton(row["label"], url=row["url"])])
        if not kb_rows and anime.get("watch_url"):
            kb_rows = [[InlineKeyboardButton("🚀 Watch / Download", url=anime["watch_url"])]]
        ...
        if img:
            try:
                await msg.reply_photo(photo=img, caption=caption, reply_markup=kb, parse_mode=enums.ParseMode.HTML); return
            except Exception: pass
    ...
    async def get_help_text():
        ...
        return (
            ...
            "/anime_list — Browse anime list\n"
            "/ping — Check bot speed\n"
            "/id — Your Telegram ID\n"
            "/report [msg] — Report to admins\n\n"
            "━━━━ 🛡️ ADMIN ━━━━\n"
            "/panel — Admin control panel\n"
            "/add_ani — Add new anime (Normal/Sticker)\n"
            "/edit_ani — Edit anime\n"
            "/delete_ani — Delete anime\n"
            "/add_alias — Add search aliases\n"
            "/list — All anime with edit/delete\n"
            ...
            "/set_main_channel_button [url] — Set Main Channel 🦋 button\n"
            "/set_help_button [url] — Set Help 🩵 button\n"
            "/set_my_name [names] — Set bot trigger names (comma separated)\n"
            "/set_gc_owner [@username] — Set GC owner for maid replies\n"
            "\n━━━━ 🔒 FORCE-SUB ━━━━\n"
            "/add_forcesub [id] — Add force-sub channel\n"
            "/rem_forcesub [id] — Remove force-sub channel\n"
            "/list_fsub — List all force-sub channels with names\n"
            ...
            "/infinite req [id] — Request-to-join (with Auto Acc ON/OFF)\n"
            "/infinite list — Your links with channel names\n"
    ...
    ALL_CMDS = [
        "start","help","search","popular","anime_list","report","cancel","panel","infinite",
        ...
        "set_welcome","set_goodbye","add_forcesub","rem_forcesub","list_fsub","add_fsubimg","forcesub_req","set_anim_img",
            ...
            if param.startswith("inf_"):
                try:
                    ...
                    if rec:
                        ...
                        # Show "Playing a game..." status while verifying
                        try:
                            await app.send_chat_action(msg.chat.id, enums.ChatAction.PLAYING)
                        except Exception:
                            pass
    ...
    async def cmd_search(_, msg: Message):
        ...
        else:     await msg.reply_text("<b><blockquote>Your anime is not available report admin using /report cmd😇</b></blockquote>", parse_mode=enums.ParseMode.HTML)
    ...
    async def build_anime_list_msg(page: int):
        total  = await anime_col.count_documents({})
        pages  = max(1, (total + PER_PAGE - 1) // PER_PAGE)
        page   = max(0, min(page, pages - 1))
        animes = await anime_col.find({}).sort("name", 1).skip(page * PER_PAGE).limit(PER_PAGE).to_list(PER_PAGE)
        lines  = "\n".join(f"{page*PER_PAGE+i+1}. {a['name']}" for i, a in enumerate(animes))
        text   = f"📋 **Anime List**\nPage {page+1}/{pages} • Total: {total}\n\n{lines}"
        nav = []
        if page > 0:       nav.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"alist_{page-1}"))
        if page < pages-1: nav.append(InlineKeyboardButton("Next ➡️", callback_data=f"alist_{page+1}"))
        rows = [nav] if nav else []
        rows.append([InlineKeyboardButton("✅ That's all!", callback_data="noop")] if not nav else [])
        rows = [r for r in rows if r]  # remove empty rows
        rows.append([InlineKeyboardButton("🔙 Back", callback_data="back_start")])
        kb = InlineKeyboardMarkup(rows)
        return text, kb

    @app.on_message(filters.command(["anime_list", "popular"]))
    async def cmd_popular(_, msg: Message):
        if not msg.from_user: return
        await reg(msg.from_user)
        if not await fsub_ok(msg): return
        text, kb = await build_anime_list_msg(0)
    ...
    async def priv_text(_, msg: Message):
        ...
        if anime:
            ...
            await msg.reply_text(
                "<b><blockquote>Your anime is not available report admin using /report cmd😇</blockquote></b>",
    ...
    async def cmd_infinite(_, msg: Message):
        ...
        if sub == "req":
            ...
            await infinite_col.update_one(
                ...
                {"$set": {"mode": "req", "auto_acc": True, "created_at": datetime.utcnow(),
            ...
            await msg.reply_text(
                ...
                f"  • User gets a <b>DM</b> when approved\n\n"
                f"⚠️ Bot must be <b>admin</b> in the channel!\n\n"
                f"🔘 <b>Auto Approve:</b> Currently <b>ON</b>",
                parse_mode=enums.ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ Auto Acc ON",  callback_data=f"autoacc_on_{cid}"),
                     InlineKeyboardButton("❌ Auto Acc OFF", callback_data=f"autoacc_off_{cid}")],
    ...
    async def on_join_request(_, req: ChatJoinRequest):
        try:
            ...
            if not rec: return  # not our request
            # Check auto_acc — default True for old records
            if not rec.get("auto_acc", True):
                return  # auto approve is OFF, do nothing
    ...
    async def cmd_delete_all(_, msg: Message):
        if not msg.from_user: return
        if not await is_admin(msg.from_user.id): await msg.reply_text(BAKA, parse_mode=enums.ParseMode.HTML); return
        total = await anime_col.count_documents({})
        await msg.reply_text(
            f"⚠️ **DELETE ALL ANIME** ⚠️\n\n"
            f"This will delete **{total} anime** permanently!\n\n"
            f"Are you absolutely sure?",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ YES, DELETE ALL", callback_data="delall_confirm1"),
                 InlineKeyboardButton("❌ Cancel",          callback_data="noop")]]))

    # ── delete_all multi-confirm callbacks ────────────
    # handled in on_cb below
    ...
    @app.on_message(filters.command("list_fsub"))
    async def cmd_list_fsub(_, msg: Message):
        if not msg.from_user: return
        if not await is_admin(msg.from_user.id): await msg.reply_text(BAKA, parse_mode=enums.ParseMode.HTML); return
        chs      = await get_fsub_channels()
        req_chs  = await gset("fsub_req_channels", [])
        if not chs:
            await msg.reply_text("📭 No force-sub channels set.", parse_mode=enums.ParseMode.HTML); return
        lines = []
        for cid in chs:
            mode = "🔑 Req" if cid in req_chs else "📢 Invite"
            try:
                chat  = await app.get_chat(cid)
                cname = chat.title or str(cid)
            except Exception:
        ...
        await msg.reply_text(
            f"🔒 <b>Force-Sub Channels ({len(chs)}):</b>\n\n" + "\n\n".join(lines),
            parse_mode=enums.ParseMode.HTML)
    ...
    async def cmd_copy(_, msg: Message):
        ...
        bid = str(tparts[0])   # always string for CLONES key consistency
        if bid in CLONES:
    ...
    async def auto_reply_gc(_, msg: Message):
        ...
        if name_triggered and not bot_mentioned:
            await msg.reply_text(
                f"<b>Goshujin-sama {gc_owner_mention}, someone is calling you 💕</b>",
                parse_mode=enums.ParseMode.HTML
            )
            return
    ...
    async def on_cb(_, q: CallbackQuery):
        ...
        if d == "delall_confirm1":
            await q.answer()
            if not await is_admin(uid): return
            total = await anime_col.count_documents({})
            await q.message.edit_text(
                f"⚠️ **SECOND CONFIRMATION** ⚠️\n\n"
                f"You are about to delete **{total} anime**.\n"
                f"This action **CANNOT** be undone!\n\n"
                f"Bot will send backup before deleting.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔥 YES, I'M SURE", callback_data="delall_confirm2"),
                     InlineKeyboardButton("❌ Cancel",         callback_data="noop")]]))
            return

        if d == "delall_confirm2":
            await q.answer()
            if not await is_admin(uid): return
            await q.message.edit_text(
                "⚠️ **FINAL CONFIRMATION** ⚠️\n\n"
                "**Last chance!** Type confirm below.\n"
                "Press DELETE to proceed:",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("💀 DELETE EVERYTHING", callback_data="delall_confirm3"),
                     InlineKeyboardButton("❌ Cancel",             callback_data="noop")]]))
            return

        if d == "delall_confirm3":
            await q.answer("Processing...")
            if not await is_admin(uid): return
            await q.message.edit_text("📦 Creating backup before deletion...")
            # Auto backup
            all_anime = await anime_col.find({}, {"_id": 0}).to_list(None)
            # JSON backup
            json_buf = io.BytesIO(json.dumps(all_anime, default=str, ensure_ascii=False, indent=2).encode())
            json_buf.name = f"anime_backup_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
            # TXT backup
            txt_lines = [f"{i+1}. {a.get('name','')}" for i, a in enumerate(all_anime)]
            txt_buf   = io.BytesIO("\n".join(txt_lines).encode())
            txt_buf.name = f"anime_backup_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.txt"
            try:
                await app.send_document(uid, json_buf, caption=f"📄 JSON Backup — {len(all_anime)} anime")
                await app.send_document(uid, txt_buf,  caption=f"📝 TXT Backup — {len(all_anime)} anime")
            except Exception as e:
                await q.message.reply_text(f"⚠️ Backup send failed: {e}\nDeletion aborted for safety.")
                return
            # Delete all
            result = await anime_col.delete_many({})
            await q.message.edit_text(
                f"✅ **Done!**\n\n"
                f"🗑️ Deleted **{result.deleted_count}** anime\n"
                f"📦 Backup sent to your DM (JSON + TXT)")
            return
        ...
        if d == "show_popular":
            ...
            except Exception: await q.message.reply_text(text, reply_markup=kb)
            return

        # ── anime list pagination ──────────────────────
        if d.startswith("alist_"):
            await q.answer()
            try:
                page = int(d.split("_")[1])
            except Exception:
                page = 0
            text, kb = await build_anime_list_msg(page)
            try:    await q.message.edit_text(text, reply_markup=kb)
            except Exception: await q.message.reply_text(text, reply_markup=kb)
            return
        ...
        # ── infinite callbacks ─────────────────────────
        if d.startswith("autoacc_on_") or d.startswith("autoacc_off_"):
            await q.answer()
            is_on = d.startswith("autoacc_on_")
            cid   = int(d.split("_")[-1])
            # Only update THIS specific channel record
            await infinite_col.update_one(
                {"channel_id": cid, "mode": "req"},
                {"$set": {"auto_acc": is_on}})
            status_txt = "✅ ON" if is_on else "❌ OFF"
            try:
                # Rebuild keyboard preserving other rows, update only toggle row
                old_kb  = q.message.reply_markup.inline_keyboard
                new_kb  = [
                    [InlineKeyboardButton(f"✅ Auto Acc ON{' ◀' if is_on else ''}",  callback_data=f"autoacc_on_{cid}"),
                     InlineKeyboardButton(f"❌ Auto Acc OFF{' ◀' if not is_on else ''}", callback_data=f"autoacc_off_{cid}")],
                ] + [row for row in old_kb if not any(
                    btn.callback_data and ("autoacc_on_" in btn.callback_data or "autoacc_off_" in btn.callback_data)
                    for btn in row
                )]
                await q.message.edit_reply_markup(InlineKeyboardMarkup(new_kb))
            except Exception: pass
            await q.answer(f"Auto Approve: {status_txt}", show_alert=True)
            return
        ...
        if d == "inf_list":
            ...
            for l in links:
                cid  = l["channel_id"]
                mode = l.get("mode","invite")
                auto = "✅" if l.get("auto_acc", True) else "❌"
                try:
                    chat = await app.get_chat(cid)
                    cname = chat.title or str(cid)
                except Exception:
                    cname = str(cid)
                link = f"https://t.me/{un}?start=inf_{cid}_{uid}"
                lines.append(f"• <b>{cname}</b>\n  <code>{cid}</code> | Mode: <b>{mode}</b>" +
                             (f" | AutoAcc: {auto}" if mode=="req" else "") +
                             f"\n  🔗 <code>{link}</code>")
                rows.append([InlineKeyboardButton(f"🗑️ Remove — {cname}", callback_data=f"inf_remove_{cid}")])
            text = "🔗 <b>Your Infinite Links:</b>\n\n" + "\n\n".join(lines)
            try:    await q.message.edit_text(text, reply_markup=InlineKeyboardMarkup(rows), parse_mode=enums.ParseMode.HTML)
            except Exception: await q.message.reply_text(text, reply_markup=InlineKeyboardMarkup(rows), parse_mode=enums.ParseMode.HTML)
            return
    ...
    async def state_fn(msg: Message):
        ...
        # ADD ANIME
            ...
            existing = await anime_col.find_one({"name_lower": d["name"].lower()})
            if existing:
                await msg.reply_text(
                    f"❌ **'{d['name']}'** already exists!\n\nDelete first, then add again.",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🗑️ Delete Existing", callback_data=f"del_ani_{existing['_id']}"),
                         InlineKeyboardButton("🎛️ Panel", callback_data="open_panel")]]))
                clr_st(uid); return
            set_st(uid, "ani_synopsis", d)
            await msg.reply_text("📝 **Step 2/3** — Send **synopsis** (or SKIP):")

        elif step == "ani_synopsis":
            t = (msg.text or "").strip()
            d["desc"] = "" if t.upper() == "SKIP" else t
            set_st(uid, "ani_buttons", d)
            await msg.reply_text(
                "🔗 **Step 3/3** — Send **inline buttons**\n\n"
                "**Format (one per line):**\n"
                "`Button Name - URL`\n\n"
                "**Same row:** Put `|` between buttons on same line:\n"
                "`EP 1-100 - https://t.me/l1 | EP 101-200 - https://t.me/l2`\n\n"
                "**Multiple rows example:**\n"
                "`⧉ EP 001-100 ⧉ - https://t.me/l1 | ⧉ EP 101-200 ⧉ - https://t.me/l2`\n"
                "`⧉ EP 201-300 ⧉ - https://t.me/l3 | ⧉ EP 301-400 ⧉ - https://t.me/l4`\n\n"
                "Each **line = one row**, use `|` to add buttons in same row.\n"
                "Type **SKIP** for no button.\n\n"
                "_/cancel to abort_")

        elif step == "ani_buttons":
            t = (msg.text or "").strip()
            buttons = []   # list of rows, each row is list of {label, url}
            if t.upper() != "SKIP":
                for line in t.splitlines():
                    ...
                    for cell in line.split("|"):
                        ...
                        if " - " in cell:
                            ...
                            lbl = parts[0].strip()
                            url = parts[1].strip()
                            if lbl and url.startswith("http"):
                                row.append({"label": lbl, "url": url})
                        elif cell.startswith("http"):
                            row.append({"label": "🚀 Watch / Download", "url": cell})
                    if row:
                        buttons.append(row)
            d["buttons"] = buttons
                ...
                await msg.reply_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🗑️ Delete Existing", callback_data=f"del_ani_{existing['_id']}"),
                         InlineKeyboardButton("🎛️ Panel", callback_data="open_panel")]]))
                return
            first_url = buttons[0][0]["url"] if buttons and buttons[0] else ""
            await anime_col.insert_one({
                "name":         d["name"],
                "name_lower":   d["name"].lower(),
                "description":  d.get("desc", ""),
                "image_file_id":d.get("img"),
                "watch_url":    first_url,
                "buttons":      buttons,
                "aliases":      [],
                "aliases_lower":[],
            ...
            btn_preview = f"\n🔗 {total_btns} button(s) in {len(buttons)} row(s)" if buttons else "\n⚠️ No buttons"
            await msg.reply_text(
                f"✅ **{d['name']}** added!{btn_preview}",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("➕ Add Another", callback_data="panel_add_ani"),
                     InlineKeyboardButton("🎛️ Panel",       callback_data="open_panel")]]))

        # old ani_watchlink / ani_aliases kept as fallback (shouldn't trigger anymore)
        elif step == "ani_watchlink":
            t = (msg.text or "").strip()
            d["url"] = "" if t.upper() == "SKIP" else t
            set_st(uid, "ani_aliases", d)
            await msg.reply_text("🏷️ **Step 4/4** — Send **aliases** (or SKIP):")
            ...
            await anime_col.insert_one({
                ...
                "watch_url": d.get("url",""), "buttons": [], "aliases": al,
        ...
        # ── STICKER MODE STEPS ─────────────────────────────
            ...
            if existing:
                await msg.reply_text(
                    f"❌ **'{d['name']}'** already exists!\n\n"
                    "Delete the existing one first, then add again.",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🗑️ Delete Existing", callback_data=f"del_ani_{existing['_id']}"),
                         InlineKeyboardButton("🎛️ Panel", callback_data="open_panel")]]))
                clr_st(uid); return
            ...
            await msg.reply_text(
                ...
                "**Example:**\n"
                "`⧉ CLICK HERE TO DOWNLOAD ⧉ - https://t.me/KENSHIN_ANIME`\n\n"
                "Or just send the **URL** alone — default button name will be used.\n"
                "Type **SKIP** for no button.")
            ...
            if t.upper() == "SKIP":
                ...
                if " - " in t:
                    parts = t.split(" - ", 1)
                    d["btn_label"] = parts[0].strip()
                    d["btn_url"]   = parts[1].strip() if parts[1].strip().startswith("http") else ""
                elif t.startswith("http"):
                    d["btn_label"] = "⧉ CLICK HERE TO DOWNLOAD ⧉"
                    d["btn_url"]   = t
                else:
                    d["btn_label"] = t
                    d["btn_url"]   = ""
            ...
            if existing:
                ...
                await msg.reply_text(
                    f"❌ **'{d['name']}'** already exists!\n\n"
                    f"Please delete the existing one first, then add again.",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🗑️ Delete Existing", callback_data=f"del_ani_{existing['_id']}"),
                         InlineKeyboardButton("🎛️ Panel", callback_data="open_panel")]]))
                return
    ...
    async def register_commands():
        ...
        user_cmds = [
            ...
            BotCommand("anime_list","🌟 Browse anime list"),
...
async def main():
    ...
    async for inst in instances_col.find({}):
        bid = str(inst["bot_id"])   # always string for consistency
        if bid in CLONES: continue
        token = inst.get("bot_token", "")
        if not token:
            logger.warning(f"⚠️ Clone {bid} has no token stored — skipping restore")
            continue
        # Validate token is still usable before attempting to start
        try:
            async with aiohttp.ClientSession() as sess:
                async with sess.get(
                    ...
                    res = await r.json()
            if not res.get("ok"):
                logger.error(f"❌ Clone {bid} token invalid: {res.get('description')} — kept in DB for manual fix")
                continue
        except Exception as e:
            logger.error(f"❌ Clone {bid} token check failed: {e} — kept in DB, will retry next restart")
            continue
        try:
            clone, clone_reg_cmds = make_bot({
                "bot_token":         token,
            ...
            CLONES[bid] = clone
            await clone_reg_cmds()
            # Normalise stored bot_id to string to prevent future type mismatch
            ...
            logger.error(f"❌ Clone restore failed ({bid}): {e} — kept in DB for next restart")
