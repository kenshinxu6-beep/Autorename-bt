...
def make_bot(cfg: dict) -> tuple:
    ...
    async def send_result(msg: Message, anime: dict):
        ...
        if anime.get("mode") == "sticker" and anime.get("sticker_id"):
            ...
            stk_msg = None
            try:
                stk_msg = await msg.reply_sticker(sticker=anime["sticker_id"])
            except Exception:
                pass
            if btn_label and btn_url:
                kb = InlineKeyboardMarkup([[InlineKeyboardButton(btn_label, url=btn_url)]])
                # Reply to the sticker itself so button appears right below it
                target = stk_msg or msg
                await target.reply_text("‌", reply_markup=kb)
            return
