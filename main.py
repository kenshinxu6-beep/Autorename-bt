...
def make_bot(cfg: dict) -> tuple:
    ...
    async def send_result(msg: Message, anime: dict):
        ...
        if anime.get("mode") == "sticker" and anime.get("sticker_id"):
            btn_label = anime.get("btn_label", "🚀 Watch / Download")
            btn_url   = anime.get("btn_url") or url
            if not btn_url or btn_url == "https://t.me/": btn_url = "https://t.me/"
            kb = InlineKeyboardMarkup([[InlineKeyboardButton(btn_label, url=btn_url)]])
            try: await msg.reply_sticker(sticker=anime["sticker_id"])
            except Exception: pass
            await msg.reply_text(
                f"✨ <b>{name.upper()}</b> ✨{promo}",
                reply_markup=kb, parse_mode=enums.ParseMode.HTML)
            return
    ...
    ALL_CMDS = [
        ...
        "set_name","set_main_channel_button","set_help_button","set_my_name",
        "genlink","batch_link","done","set_autodelete",
    ]
    ...
    async def cmd_start(_, msg: Message):
        ...
        if msg.text and len(msg.text.split()) > 1:
            ...
            if param.startswith("gl_"):
                gl_key = param[3:]
                rec = await db["genlinks"].find_one({"key": gl_key})
                if not rec:
                    await msg.reply_text("❌ Link invalid or expired."); return
                await run_anim(msg)
                chs = await get_fsub_channels()
                if chs:
                    failed = await fsub_check_user(msg.from_user.id)
                    if failed:
                        me2  = await get_me_cached()
                        orig = f"https://t.me/{me2.username}?start=gl_{gl_key}"
                        await send_fsub_prompt(msg, failed, inf_link=orig); return
                fid   = rec["file_id"]
                ftype = rec.get("ftype","document")
                expire_secs = rec.get("expire_secs", 1800)
                warn = (
                    f"<b>‼️⚠️IMPORTANT ⚠️‼️\n\n</b>"
                    f"<blockquote><b>ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ɪɴ {expire_secs//60} ᴍɪɴᴜᴛᴇ(s)🫥 "
                    f"(ᴅᴜᴇ ᴛᴏ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs).</b></blockquote>"
                    f"<b>\n\n</b><blockquote><b>ᴘʟᴇᴀsᴇ ғᴏʀᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ ʏᴏᴜʀ sᴀᴠᴇᴅ ᴍᴇssᴀɢᴇs "
                    f"ᴏʀ ᴀɴʏ ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ ᴛᴏ ᴀᴠᴏɪᴅ ʟᴏsɪɴɢ.</b></blockquote>"
                )
                try:
                    if ftype=="photo":       s=await msg.reply_photo(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                    elif ftype=="video":     s=await msg.reply_video(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                    elif ftype=="sticker":   s=await msg.reply_sticker(fid)
                    elif ftype=="audio":     s=await msg.reply_audio(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                    elif ftype=="animation": s=await msg.reply_animation(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                    elif ftype=="text":      s=await msg.reply_text(f"{warn}\n\n{fid}", parse_mode=enums.ParseMode.HTML)
                    else:                    s=await msg.reply_document(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                    asyncio.create_task(_autodelete(s, expire_secs))
                except Exception as e:
                    await msg.reply_text(f"❌ Error: {e}")
                return

            if param.startswith("bl_"):
                bl_key = param[3:]
                rec = await db["batchlinks"].find_one({"key": bl_key})
                if not rec:
                    await msg.reply_text("❌ Batch link invalid or expired."); return
                await run_anim(msg)
                chs = await get_fsub_channels()
                if chs:
                    failed = await fsub_check_user(msg.from_user.id)
                    if failed:
                        me2  = await get_me_cached()
                        orig = f"https://t.me/{me2.username}?start=bl_{bl_key}"
                        await send_fsub_prompt(msg, failed, inf_link=orig); return
                expire_secs = rec.get("expire_secs", 1800)
                warn = (
                    f"<b>‼️⚠️IMPORTANT ⚠️‼️\n\n</b>"
                ...
                sent_msgs = []
                for i, item in enumerate(sorted(rec["files"], key=lambda x: x.get("idx",0)), 1):
                    fid   = item["file_id"]
                    ftype = item.get("ftype","document")
                    cap   = warn if i == 1 else None
                    try:
                        if ftype=="photo":       s=await msg.reply_photo(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                        elif ftype=="video":     s=await msg.reply_video(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                        elif ftype=="sticker":   s=await msg.reply_sticker(fid)
                        elif ftype=="audio":     s=await msg.reply_audio(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                        elif ftype=="animation": s=await msg.reply_animation(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                        else:                    s=await msg.reply_document(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                        sent_msgs.append(s)
    ...
    # ── Auto-delete helper ─────────────────────────────
    async def _autodelete(msg: Message, seconds: int):
    ...
    async def cmd_set_main_ch(_, msg: Message):
        ...
        if not await is_admin(msg.from_user.id): await msg.reply_text(BAKA, parse_mode=enums.ParseMode.HTML); return
        parts = (msg.text or "").split(None, 1)
        if len(parts) < 2: await msg.reply_text("Usage: `/set_main_channel_button https://t.me/yourchannel`"); return
    ...
    async def cmd_set_help_btn(_, msg: Message):
        ...
        if not await is_admin(msg.from_user.id): await msg.reply_text(BAKA, parse_mode=enums.ParseMode.HTML); return
        parts = (msg.text or "").split(None, 1)
        if len(parts) < 2: await msg.reply_text("Usage: `/set_help_button https://t.me/yourhelpchannel`"); return
    ...
    # ── /set_autodelete ───────────────────────────────
    @app.on_message(filters.command("set_autodelete"))
    async def cmd_set_autodelete(_, msg: Message):
        if not msg.from_user: return
        if not await is_admin(msg.from_user.id): await msg.reply_text(BAKA, parse_mode=enums.ParseMode.HTML); return
        parts = (msg.text or "").split()
        if len(parts) < 2 or not parts[1].isdigit():
            cur = await gset("genlink_expire", 1800)
            await msg.reply_text(f"Current auto-delete: **{cur//60} minutes**\n\nUsage: `/set_autodelete 30` (minutes)"); return
        minutes = int(parts[1])
        await sset("genlink_expire", minutes * 60)
        await msg.reply_text(f"✅ Auto-delete set to **{minutes} minutes** for all new genlinks/batchlinks!")

    # ── /genlink ──────────────────────────────────────
    @app.on_message(filters.command("genlink"))
    async def cmd_genlink(_, msg: Message):
        if not msg.from_user: return
        if not await is_admin(msg.from_user.id): await msg.reply_text(BAKA, parse_mode=enums.ParseMode.HTML); return
        expire = await gset("genlink_expire", 1800)
        set_st(msg.from_user.id, "genlink_wait", {"expire_secs": expire})
        await msg.reply_text(
            "📁 **Generate File Link**\n\n"
            "Send any **file, video, photo, GIF, sticker or text**.\n"
            "Bot will generate a sharable link with FSub + auto-delete.\n\n"
            f"⏰ Auto-delete: **{expire//60} min** (change with `/set_autodelete`)\n\n"
            "_/cancel to abort_")

    # ── /batch_link ───────────────────────────────────
    @app.on_message(filters.command("batch_link"))
    async def cmd_batch_link(_, msg: Message):
        if not msg.from_user: return
        if not await is_admin(msg.from_user.id): await msg.reply_text(BAKA, parse_mode=enums.ParseMode.HTML); return
        expire = await gset("genlink_expire", 1800)
        set_st(msg.from_user.id, "batch_wait", {"files": [], "expire_secs": expire})
        await msg.reply_text(
            "📦 **Batch Link — Send Files**\n\n"
            "Send files one by one. Bot stores them in order.\n"
            "Type **/done** when finished — bot will generate ONE link.\n\n"
            f"⏰ Auto-delete: **{expire//60} min**\n\n"
            "_/cancel to abort_")

    # ── /done (finalize batch) ────────────────────────
    @app.on_message(filters.command("done"))
    async def cmd_done(_, msg: Message):
        if not msg.from_user: return
        uid = msg.from_user.id
        st  = get_st(uid)
        if not st or st["step"] != "batch_wait":
            await msg.reply_text("No active batch session. Start with /batch_link"); return
        d     = st["data"]
        files = d.get("files", [])
        if not files:
            await msg.reply_text("❌ No files added yet!"); return
        import secrets
        key         = secrets.token_urlsafe(8)
        expire_secs = d.get("expire_secs", 1800)
        me          = await get_me_cached()
        await db["batchlinks"].insert_one({
            "key": key, "files": files, "expire_secs": expire_secs,
            "added_by": uid, "added_at": datetime.utcnow(),
        })
        link = f"https://t.me/{me.username}?start=bl_{key}"
        clr_st(uid)
        await msg.reply_text(
            f"✅ **Batch Link Generated! ({len(files)} files)**\n\n"
            f"🔗 `{link}`\n\n"
            f"⏰ Auto-delete: **{expire_secs//60} min** per file\n"
            f"📋 Files sorted in send order (1→{len(files)})\n"
            f"🔒 FSub check: enabled",
    ...
    async def auto_reply_gc(_, msg: Message):
        ...
        GREETING = ["hello","hi","hey","hello bot","tum kon","who are you","kon ho","kya ho","namaste"]
        is_greeting = any(textl == g or textl.startswith(g+" ") for g in GREETING)

        master_ids_list = await staff_ids()
        is_master = uid in master_ids_list

        # ── Trigger name in group ──────────────────────
        if name_triggered and not bot_mentioned:
            if is_master:
                replies = [
                    f"Goshujin-sama~ 💕 You called for me?",
                    f"Hai, Goshujin-sama! At your service! 🥰",
                ]
            else:
                replies = [
                    f"Goshujin-sama @{(me.username or 'master')}, someone is calling you~ 💛",
                    f"Oh? Someone said my master's name~ Go ahead! 🤭",
                ]
            await msg.reply_text(_random.choice(replies))
            return

        # ── Bot mentioned + greeting ───────────────────
        if bot_mentioned and is_greeting:
            if is_master:
                replies = [
                    "Welcome back, Goshujin-sama! ❤️ I have been waiting for you all day. Let me serve you right away! 🥰",
                    "Ah, Goshujin-sama! Hello! ✨ Seeing your message makes my heart flutter~ 🫖",
                ]
            else:
                replies = [
                    f"Greetings. I am the personal maid of Lord @{me.username or 'master'} 💛. I hope you are having a pleasant day.",
                    f"Hello to you too. Anyway... Goshujin-sama @{me.username or 'master'}, your guests are being noisy today~ 🤭",
                ]
            await msg.reply_text(_random.choice(replies))
            return

        # ── Bot mentioned but not greeting → anime search ─
        if bot_mentioned:
    ...
    async def on_cb(_, q: CallbackQuery):
        ...
        if d.startswith("gl_check_"):
            await q.answer("🔄 Checking…")
            gl_key = d[9:]
            rec = await db["genlinks"].find_one({"key": gl_key})
            if not rec:
                await q.message.reply_text("❌ Link expired or invalid."); return
            failed = await fsub_check_user(uid)
            if failed:
                await q.answer("❌ Still not joined!", show_alert=True); return
            # Send file
            fid   = rec["file_id"]
            ftype = rec.get("ftype", "document")
            expire_secs = rec.get("expire_secs", 1800)
            warn = (
                f"<b>‼️⚠️IMPORTANT ⚠️‼️\n\n</b>"
                f"<blockquote><b>ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ɪɴ "
                f"{expire_secs//60} ᴍɪɴᴜᴛᴇ(s)🫥 (ᴅᴜᴇ ᴛᴏ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs).</b></blockquote>"
                f"<b>\n\n</b><blockquote><b>ᴘʟᴇᴀsᴇ ғᴏʀᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ ʏᴏᴜʀ sᴀᴠᴇᴅ ᴍᴇssᴀɢᴇs "
                f"ᴏʀ ᴀɴʏ ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ ᴛᴏ ᴀᴠᴏɪᴅ ʟᴏsɪɴɢ.</b></blockquote>"
            )
            try:
                if ftype == "photo":    sent = await q.message.reply_photo(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                elif ftype == "video":  sent = await q.message.reply_video(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                elif ftype == "sticker":sent = await q.message.reply_sticker(fid)
                elif ftype == "audio":  sent = await q.message.reply_audio(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                elif ftype == "animation": sent = await q.message.reply_animation(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                else:                   sent = await q.message.reply_document(fid, caption=warn, parse_mode=enums.ParseMode.HTML)
                asyncio.create_task(_autodelete(sent, expire_secs))
            except Exception as e:
                await q.message.reply_text(f"❌ Error sending file: {e}")
            return

        if d.startswith("bl_check_"):
            await q.answer("🔄 Checking…")
            bl_key = d[9:]
            rec = await db["batchlinks"].find_one({"key": bl_key})
            if not rec:
                await q.message.reply_text("❌ Batch link expired or invalid."); return
            failed = await fsub_check_user(uid)
            if failed:
                await q.answer("❌ Still not joined!", show_alert=True); return
            expire_secs = rec.get("expire_secs", 1800)
            ...
            for i, item in enumerate(sorted(rec["files"], key=lambda x: x.get("idx", 0)), 1):
                ...
                cap   = warn if i == 1 else None
                try:
                    if ftype == "photo":    s = await q.message.reply_photo(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                    elif ftype == "video":  s = await q.message.reply_video(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                    elif ftype == "sticker":s = await q.message.reply_sticker(fid)
                    elif ftype == "audio":  s = await q.message.reply_audio(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                    elif ftype == "animation": s = await q.message.reply_animation(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                    else:                   s = await q.message.reply_document(fid, caption=cap, parse_mode=enums.ParseMode.HTML)
                    sent_msgs.append(s)
                    await asyncio.sleep(0.3)
                except Exception as e:
                    logger.error(f"batch send error: {e}")
    ...
    async def state_fn(msg: Message):
        ...
        # ── STICKER MODE STEPS ─────────────────────────────
            ...
            set_st(uid, "stk_link", d)
            await msg.reply_text(
                "🔗 **Step 3/5** — Send the **Watch/Download link** (or SKIP):\n\n"
                "This is the URL inside the button.")
            ...
            await msg.reply_text(
                "🎨 **Step 4/5** — Send **inline button config**:\n\n"
                "**Format:** `Button Name - URL - style:color`\n\n"
                "**Example:**\n"
                "`⧉ CLICK HERE TO DOWNLOAD ⧉ - https://t.me/bot?start=xyz - style:red`\n\n"
                "**Colors:** red, blue, green, orange\n"
                "If no color → simple button\n"
                "Type **SKIP** to use default button with URL from step 3.")

        elif step == "stk_btn":
            t = (msg.text or "").strip()
            if t.upper() == "SKIP":
                d["btn_label"] = "🚀 Watch / Download"
                d["btn_url"]   = d.get("url","")
                d["btn_color"] = None
            else:
                # Format: "Label - URL - style:color"  or  "Label - style:color"
                ...
                for p in parts[1:]:
                    if p.startswith("style:"):
                        d["btn_color"] = p[6:].strip()
                    elif p.startswith("http"):
                        d["btn_url"] = p
            set_st(uid, "stk_aliases", d)
            preview_color = f" | color:{d['btn_color']}" if d.get("btn_color") else ""
            await msg.reply_text(
                f"✅ Button: **{d['btn_label']}**{preview_color}\n\n"
                "🏷️ **Step 5/5** — Send **aliases** comma-separated (or SKIP):\n"
        ...
        # ── GENLINK STATE ──────────────────────────────────
        elif step == "genlink_wait":
            # Accept any media or text
            fid, ftype = None, "document"
            if msg.photo:      fid = msg.photo.file_id;     ftype = "photo"
            elif msg.video:    fid = msg.video.file_id;     ftype = "video"
            elif msg.document: fid = msg.document.file_id;  ftype = "document"
            elif msg.audio:    fid = msg.audio.file_id;     ftype = "audio"
            elif msg.sticker:  fid = msg.sticker.file_id;   ftype = "sticker"
            elif msg.animation:fid = msg.animation.file_id; ftype = "animation"
            elif msg.text:     fid = msg.text;               ftype = "text"
            if not fid:
                await msg.reply_text("Send any file/media/text to generate link."); return
            import secrets
            key          = secrets.token_urlsafe(8)
            expire_secs  = d.get("expire_secs", 1800)
            me           = await get_me_cached()
            await db["genlinks"].insert_one({
                "key": key, "file_id": fid, "ftype": ftype,
                "expire_secs": expire_secs, "added_by": uid,
                "added_at": datetime.utcnow(),
            })
            link = f"https://t.me/{me.username}?start=gl_{key}"
            clr_st(uid)
            await msg.reply_text(
                f"✅ **Link Generated!**\n\n"
                f"🔗 `{link}`\n\n"
                f"⏰ Auto-delete: **{expire_secs//60} min** after user opens\n"
                f"🔒 FSub check: enabled",
                parse_mode=enums.ParseMode.MARKDOWN)

        # ── BATCH LINK STATE ───────────────────────────────
        elif step == "batch_wait":
            fid, ftype = None, "document"
            if msg.photo:      fid = msg.photo.file_id;     ftype = "photo"
            elif msg.video:    fid = msg.video.file_id;     ftype = "video"
            elif msg.document: fid = msg.document.file_id;  ftype = "document"
            ...
            if not fid:
                await msg.reply_text("Send files/media. Type /done when finished."); return
            files = d.get("files", [])
            files.append({"file_id": fid, "ftype": ftype, "idx": len(files)+1})
            d["files"] = files
            set_st(uid, "batch_wait", d)
            await msg.reply_text(f"✅ File {len(files)} added. Send more or /done.")
