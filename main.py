# ═══════════════════════════════════════════════════════════
# RENAMER BOT v3.5 – PyroFork, Fixed await, Max Speed
# ═══════════════════════════════════════════════════════════
...
app = Client(
    ...
    api_hash=API_HASH,
    bot_token=BOT_TOKEN,
    workers=500,           # Max concurrent workers
    max_concurrent_transmissions=10,  # Parallel downloads
    in_memory=True,
    sleep_threshold=10,     # Reduce delay
    retries=3,
    timeout=120
)
...
async def get_semaphore(uid):
    ...
    if admin_batch_mode and (uid==OWNER_ID or await is_admin(uid)): lim=100
        ...
        lim = s.get("max_concurrent_admin", 100) if s else 100
    elif uid==OWNER_ID or await is_admin(uid):
        s = await get_settings()
        lim = s.get("max_concurrent_admin", 100) if s else 100
    else:
        s = await get_settings()
        lim = s.get("max_concurrent_normal", 10) if s else 10
...
async def cb_start(c, q):
    ...
    default_msg = "👋 **Welcome {username}!**"
    text = await get_global_setting("start_message", default_msg)
...
async def cb_about(c, q):
    await q.answer()
    await q.message.edit_text(f"**Renamer Bot v3.5**\nOwner: @{OWNER_USERNAME}")
...
async def cb_ver(c, q):
    if await check_force_sub(q.from_user.id):
        ...
        await q.message.edit_text("✅ Send a video.")
    else:
...
async def st_cmd(c, m): await m.reply("✅ Bot running")
...
# ═══ Owner Commands (ALL - Same as before) ═══
...
async def addtoken(c, m):
    ...
    await add_bot_token(p[1], int(p[2]))
...
async def adddump(c, m):
    ...
    await add_dump_channel(int(p[1]), p[2])
...
async def setfsub(c, m):
    ...
    if ch not in lst:
        lst.append(ch)
        await set_global_setting("fsub_channels", lst)
...
# ═══ RENAME HANDLER (Max Speed Download, Temp File, Single Upload) ═══
    ...
    async with sem:
        ...
        try:
            ...
            if m.reply_to_message and m.reply_to_message.photo:
                thumb = await m.reply_to_message.download()
            if not thumb:
